...
const styles = {
  ...
  controlContainer: {
    ...
    borderRadius: "24px",
    boxShadow:
      "0 4px 16px rgba(100, 116, 139, 0.08), 0 1px 4px rgba(100, 116, 139, 0.04)",
    ...
    >
      ...
      >
        ...
        <div style={{ marginBottom: "8px" }}>
          Real-time health monitoring for all ARTAgent backend services
          including Redis cache, Azure OpenAI, Speech Services, and
    ...
    >
      ...
      >
        ...
        <div style={styles.helpTooltipText}>
          ARTAgent is an accelerator that delivers a friction-free, AI-driven
          voice experience—whether callers dial a phone number, speak to an IVR,
          or click "Call Me" in a web app. Built entirely on Azure services, it
          provides a low-latency stack that scales on demand while keeping the
          AI layer fully under your control.
        </div>
        <div style={styles.helpTooltipText}>
          Design a single agent or orchestrate multiple specialist agents. The
          framework allows you to build your voice agent from scratch,
          incorporate memory, configure actions, and fine-tune your TTS and STT
          layers.
        </div>
        <div style={styles.helpTooltipText}>
          🤔 <strong>Try asking about:</strong> Insurance claims, policy
          questions, authentication, or general inquiries.
        </div>
        <div style={styles.helpTooltipText}>
          📑{" "}
          <a
...
const BackendIndicator = ({ url, onConfigureClick, onStatusChange }) => {
  ...
  const unhealthyChecks = readinessChecks.filter(
    ...
    >
      ...
      {(shouldBeExpanded || overallStatus !== "healthy") && (
        <>
          ...
          {shouldBeExpanded && (
            <>
              ...
              >
                ...
                >
                  Main FastAPI server handling WebSocket connections, voice
                  processing, and AI agent orchestration
                </div>
              </div>
                  ...
                  >
                    ...
                    <div
                      style={{
                        fontSize: "12px",
                        color: "#64748b",
                        transform: showComponentDetails
                          ? "rotate(180deg)"
                          : "rotate(0deg)",
                        transition: "transform 0.2s ease",
                      }}
              ...
              {acsOnlyIssue && showAcsHover && acsTooltipPos && (
                ...
                >
                  ACS outbound calling is currently unavailable, but the
                  Conversation API continues to stream microphone audio from
                  this device to the backend.
                </div>
              )}
            </>
          )}
          ...
          {error ? (
            <div style={styles.errorMessage}>⚠️ Connection failed: {error}</div>
              ...
              <div style={styles.componentGrid}>
                {readinessData.checks.map((check, idx) => (
                  <div
                    ...
                    title={
                      check.details ||
                      `${check.component} status: ${check.status}`
                    }
                  >
                    <div
                      style={{
                    ...
                    {shouldBeExpanded && (
                      ...
                      >
                        {componentDescriptions[check.component] ||
          ...
          {readinessData?.response_time_ms && shouldBeExpanded && (
            ...
            >
              <span>
                ...
                {readinessData.response_time_ms.toFixed(0)}ms
              </span>
                ...
                {healthData.session_metrics?.disconnected !== undefined && (
                  <div
                    style={{
                      ...
                      padding: "6px 8px",
                      textAlign: "center",
                      gridColumn: healthData.session_metrics
                        ? "1 / -1"
                        : "auto",
                    }}
                  >
          ...
          {shouldBeExpanded && agentsData?.agents && (
            ...
            >
              ...
              >
                {agentsData.agents.map((agent, idx) => (
                  <div
                    ...
                    title={
                      agent.description ||
                      `${agent.name} - Real-time voice agent`
                    }
                  >
                    <div
                      style={{
                      ...
                      >
                        <span
                          style={{
                            ...
                            borderRadius: "50%",
                            backgroundColor:
                              agent.status === "loaded" ? "#10b981" : "#ef4444",
              ...
              >
                Runtime configuration • Changes require restart for persistence
                • Contact rtvoiceagent@microsoft.com
              </div>
            </div>
          )}
        </>
...
const WaveformVisualization = ({
      ...
      setAmplitude(() => {
        ...
        if (audioLevel > 0.01) {
          ...
          const smoothVariation =
            Math.sin(Date.now() * 0.002) * (scaledLevel * 0.2);
          ...
          const smoothVariation =
            Math.sin(Date.now() * 0.0018) * (scaledLevel * 0.25);
  ...
  const generateWavePath = () => {
    ...
    for (let i = 0; i <= points; i++) {
      ...
      const y =
        centerY + Math.sin(x * frequency + waveOffset * 0.1) * amplitude;
      path += ` L ${x} ${y}`;
    }
  ...
  const generateSecondaryWave = () => {
    ...
    for (let i = 0; i <= points; i++) {
      ...
      const y =
        centerY +
        Math.sin(x * frequency + waveOffset * 0.12) * (amplitude * 0.6);
      path += ` L ${x} ${y}`;
    }
  ...
  return (
    <div style={isUser ? styles.userMessage : styles.assistantMessage}>
      ...
      {!isUser && (isSpecialist || isAuthAgent) && (
        <div style={styles.agentNameLabel}>{speaker}</div>
      )}
  ...
  */
  ...
  const initializeAudioPlayback = async () => {
    ...
    try {
      ...
      logger.info(
        "AudioWorklet playback system initialized, context sample rate:",
        audioCtx.sampleRate
      );
    } catch (error) {
  ...
  const { interruptAssistantOutput, recordBargeInEvent, finalizeBargeInClear } =
    useBargeIn({
      appendLog,
      setActiveSpeaker,
  ...
  const registerAssistantStreaming = useCallback(
    (speaker) => {
      const metrics = metricsRef.current;
      const now = performance.now();
      let turn = metrics.turns
        .slice()
        .reverse()
        .find((t) => !t.firstTokenTs || !t.audioEndTs);
      if (!turn) {
        const turnId = metrics.turnCounter + 1;
        metrics.turnCounter = turnId;
        turn = {
      ...
      if (!turn.firstTokenTs) {
        ...
        turn.firstTokenLatencyMs =
          turn.userTs != null ? now - turn.userTs : undefined;
  ...
  const registerAssistantFinal = useCallback(
    (speaker) => {
      const metrics = metricsRef.current;
      const now = performance.now();
      const turn = metrics.turns
        .slice()
        .reverse()
        .find((t) => !t.finalTextTs);
      if (!turn) {
        return;
      }
  ...
  const registerAudioFrame = useCallback(
    (frameIndex, isFinal) => {
      ...
      let turn =
        preferredId != null
          ? metrics.turns.find((t) => t.id === preferredId)
          : undefined;
      if (!turn) {
        turn = metrics.turns
          .slice()
      ...
      if ((frameIndex ?? 0) === 0 && turn.audioStartTs == null) {
        ...
        const deltaFromFinal =
          turn.finalTextTs != null ? now - turn.finalTextTs : undefined;
      ...
      if (isFinal) {
        turn.audioEndTs = now;
        turn.audioPlaybackDurationMs =
          turn.audioStartTs != null ? now - turn.audioStartTs : undefined;
        turn.totalLatencyMs =
          turn.userTs != null ? now - turn.userTs : undefined;
  ...
  const startRecognition = async () => {
    ...
    logger.info(
      "🔗 [FRONTEND] Starting conversation WebSocket with session_id:",
      sessionId
    );

    const connectSocket = (isReconnect = false) => {
      const ws = new WebSocket(
        `${WS_URL}/api/v1/realtime/conversation?session_id=${sessionId}`
      );
      ...
      ws.onopen = () => {
        appendLog(
          isReconnect
            ? "🔌 WS reconnected - Connected to backend!"
      ...
      ws.onclose = (event) => {
        ...
        appendLog(
          `🔄 WS reconnect scheduled in ${Math.round(
            delay
          )} ms (attempt ${attempt})`
        );
    ...
    if (sourceSampleRate !== TARGET_SAMPLE_RATE) {
      appendLog(
        `⚠️ Mic sample rate is ${sourceSampleRate}Hz, resampling to ${TARGET_SAMPLE_RATE}Hz for backend compatibility`
      );
      logger.warn(
        "Mic AudioContext sample rate mismatch: %d -> resampling to %d",
        sourceSampleRate,
        TARGET_SAMPLE_RATE
      );
    } else {
      logger.info("Mic AudioContext locked to %dHz", TARGET_SAMPLE_RATE);
    }
    ...
    processor.onaudioprocess = (evt) => {
      ...
      const normalizedSamples =
        sourceSampleRate === TARGET_SAMPLE_RATE
          ? float32
          : resampleFloat32(float32, sourceSampleRate, TARGET_SAMPLE_RATE);
      ...
      const int16 = new Int16Array(normalizedSamples.length);
      for (let i = 0; i < normalizedSamples.length; i++) {
        int16[i] = Math.max(-1, Math.min(1, normalizedSamples[i])) * 0x7fff;
      }
  ...
  const handleSocketMessage = async (event) => {
    ...
    if (typeof event.data === "string") {
      ...
      logger.debug(
        "📨 Binary WebSocket message received, length:",
    ...
    if (payload.type && payload.sender && payload.payload && payload.ts) {
      ...
      if (envelopeType === "event" && actualPayload.message) {
        ...
        envelopeType === "assistant_streaming" &&
    ...
    if (payload.event_type === "stt_partial" && payload.data) {
      ...
      if (shouldClearPlayback) {
        interruptAssistantOutput(partialMeta, {
          logMessage:
            "🔇 Audio cleared due to live speech (partial transcription)",
        });
    ...
    if (payload.type === "audio_data" && payload.data) {
      try {
        ...
        logger.debug(
          `🔊 Processing TTS audio chunk: ${
            float32.length
          } samples, sample_rate: ${payload.sample_rate || 16000}`
        );
        ...
        if (pcmSinkRef.current) {
          ...
          if (
            ...
            sourceRate &&
            playbackCtx.sampleRate !== sourceRate
          ) {
            samples = resampleFloat32(
              float32,
              sourceRate,
            ...
            if (!resampleWarningRef.current) {
              appendLog(
                `🎚️ Resampling audio ${sourceRate}Hz → ${playbackCtx.sampleRate}Hz`
              );
          ...
          appendLog(
            `🔊 TTS audio frame ${payload.frame_index + 1}/${
    ...
    if (
      ...
      setMessages((prev) => {
        const latest = prev.at(-1);
        if (latest?.streaming && latest?.speaker === assistantSpeaker) {
          return prev.map((m, i) =>
            i === prev.length - 1
              ? {
                  ...m,
    ...
    if (type === "control") {
      ...
      if (action === "tts_cancelled" || action === "audio_stop") {
        logger.debug(
          `🔇 Control audio stop received (${action}) - clearing audio queue`
        );
        const meta = {
  ...
  const startACSCall = async () => {
    if (systemStatus.status === "degraded" && systemStatus.acsOnlyIssue) {
      appendLog(
        "🚫 Outbound calling disabled until ACS configuration is provided."
      );
      return;
    }
    ...
    try {
      ...
      logger.info(
        "📞 [FRONTEND] Initiating phone call with session_id:",
        currentSessionId
      );
      logger.debug(
      ...
      logger.info(
        "🔗 [FRONTEND] Starting dashboard relay WebSocket to monitor session:",
        currentSessionId
      );
      const relay = new WebSocket(
        `${WS_URL}/api/v1/realtime/dashboard/relay?session_id=${currentSessionId}`
      );
        ...
        <div style={styles.appHeader}>
          <div style={styles.appTitleContainer}>
            ...
            <p style={styles.appSubtitle}>
              Transforming customer interactions with real-time, intelligent
              voice interactions
            </p>
            <div
              style={{
          ...
          <div style={styles.controlContainer}>
            ...
            <div style={{ position: "relative" }}>
              <button
                ...
                onClick={() => {
                  ...
                  setShowPhoneInput(false);
                  appendLog(
                    `🔄️ Session reset - new session ID: ${
                      newSessionId.split("_")[1]
                    }`
                  );
            ...
            >
              ...
              {isCallDisabled && showPhoneTooltip && phoneDisabledPos && (
                ...
                >
                  ⚠️ Outbound calling is disabled. Update backend .env with
                  Azure Communication Services settings (ACS_CONNECTION_STRING,
                  ACS_SOURCE_PHONE_NUMBER, ACS_ENDPOINT) to enable this feature.
                </div>
              )}
            </div>
          </div>
